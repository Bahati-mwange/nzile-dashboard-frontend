    ...
    return (
        <div className="container mx-auto p-6 space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
                ...
                {/* Informations de l'usager (permis de conduire) */}
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                    ...
                    <CardContent className="space-y-4">
                        {/* On suppose que l'infraction principale liée à la transaction contient les infos conducteur */}
                        {(() => {
                            const infraction = mockData.infractions.find(i => i.id === transaction.infractions[0]);
                            if (!infraction) return <span className="text-gray-500">Aucune donnée de permis trouvée</span>;
                            // Recherche du permis dans mockData.licenses
                            const license = mockData.licenses.find(l => l.driverName === infraction.driverName);
                            return (
                                <>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium text-gray-500">Nom:</span>
                                        <span className="font-medium">{infraction.driverName}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium text-gray-500">Numéro de permis:</span>
                                        <span className="font-mono font-medium">{license ? license.number : 'N/A'}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium text-gray-500">Type:</span>
                                        <span>{license ? license.type : 'N/A'}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium text-gray-500">Date de délivrance:</span>
                                        <span>{license ? license.issueDate : 'N/A'}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium text-gray-500">Expiration:</span>
                                        <span>{license ? license.expiryDate : 'N/A'}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium text-gray-500">Statut:</span>
                                        <span>{license ? license.status : 'N/A'}</span>
                                    </div>
                                </>
                            );
                        })()}
